...
Guidance for Claude Code (and any AI agent) working in this repo. These rules are
...
3. **Run `codex:review` before every push/commit.** Invoke the Codex review on your diff
   and resolve its findings *before* you commit and push. No push without a clean review.
...
- [ ] Coverage ≥ 99% (`pytest --cov=app --cov=watch --cov=scheduler --cov=export --cov-fail-under=99`)
...
```bash
pytest --cov=app --cov=watch --cov=scheduler --cov=export --cov-fail-under=99 --cov-report=term-missing   # unit + coverage gate
pytest tests/e2e                                                                               # Playwright e2e
python3 app.py                                                                                 # run locally
python scheduler.py                                                                            # run price-watch checker
```
