...
1. **RapidAPI flights-sky / Google Flights** (`RAPIDAPI_KEY`, host `RAPIDAPI_HOST`) — **PRIMARY** provider (`google_flights_fare`, tried FIRST). Uses the **synchronous** `/google/flights/search-roundtrip` endpoint: ONE call returns everything in ~13s, **no `sessionId`/poll** — it bypasses the async Skyscanner flow entirely. Params are plain IATA (`departureId`/`arrivalId`), `departureDate`/`arrivalDate`, `currency=CAD`, `adults`, and `children` as an **integer COUNT** (not ages; only sent when > 0). `price` is the **party total** round-trip fare (verified: adults=1→2485, adults=2→5208, 2 adults+2 children→8987 CAD), used as-is. Parses `data.topFlights[] + data.otherFlights[]`, keeps numerically-priced items, normalizes the cheapest into the shared dict: `stops`/`duration_min` are the **outbound** leg, `airlines` from `airlineNames` (fallback `airline[].airlineName`), nonstop fields from `stops == 0` items, and `layovers` from the cheapest's `segments` (each `arrivalAirportCode` except the last, `duration_min` None; or `transferAirports` when present). A generous `(connect=10, read=40)` timeout covers the ~13s call. `status` != true / `data` not a dict / no priced flights / missing key → `None` (never fabricated). `source="google"`, `book=None`. Same key/subscription also powers the Skyscanner fallback (#2 below).
2. **RapidAPI flights-sky / Skyscanner** (`RAPIDAPI_KEY`, host `RAPIDAPI_HOST`) — async fallback; **richest data** (round-trip duration, airlines, layovers). Place IDs are plain IATA codes (no resolution step). 3-step async flow: `search-roundtrip` → poll `search-incomplete` until the session is `complete` → parse `itineraries.buckets[].items[]`. The initial `search-roundtrip` call is itself slow to return its session (~25-30s for real long-haul, sometimes more) but the response keeps progressing the whole time, so it uses a `(connect, read)` timeout tuple: a short `SKYSCANNER_CONNECT_TIMEOUT` (default 10s) bounds connection setup so a **dead host fails fast**, and a generous `SKYSCANNER_SEARCH_TIMEOUT` (default 90s) is a **read / inactivity** timeout that fires only after that many seconds of **no data arriving** — **not** a total wall-clock cap. As long as flights-sky keeps the response progressing the app keeps waiting; it gives up only after prolonged silence (the old tight cap dropped real long-haul results every time). The poll budget is longer-but-bounded and env-configurable (`SKYSCANNER_POLL_ATTEMPTS` × `SKYSCANNER_POLL_INTERVAL`, default ~12 × 1.5s ≈ 18s) so most routes — incl. long-haul — finish instead of falling back to serpapi; a per-poll `SKYSCANNER_POLL_TIMEOUT` (default 8s) plus break-on-timeout keep a hung session from running for minutes. Prices are party totals in CAD for the passed `adults`. Transient HTTP 502 on the request is retried a couple times; any other non-200, missing key, or unexpected shape → `None`. `airlines` (cheapest item's marketing carrier names) and `layovers` (connection IATA per leg) ride in the normalized dict for surfacing in the UI.
3. **SerpApi / Google Flights** (`SERPAPI_KEY`) — live Google Flights results; best coverage for long-haul exact-date searches (e.g. Toronto → China Dec 2026). Prices are party totals in CAD (not per-ticket). No direct booking URL; falls back to Kayak link. The single-call round-trip response exposes only the outbound leg, so `nonstop_cad` is not populated for this provider (no false round-trip nonstop), `stops` reflects the outbound leg only, and `duration_min` is `null` (total round-trip duration unavailable; a future return-leg fetch could populate it).
4. **Amadeus Self-Service** (`AMADEUS_CLIENT_ID` + `AMADEUS_CLIENT_SECRET`) — test environment; limited inventory.
5. **Travelpayouts / Aviasales** (`TRAVELPAYOUTS_TOKEN`) — cached market fares; per-ticket price scaled to party total.
6. **Kiwi / Tequila** (`KIWI_API_KEY`) — real fares with booking deep-links.
...
| `RAPIDAPI_KEY` | RapidAPI key for the flights-sky API — powers BOTH the **Google Flights** synchronous endpoint (`google_flights_fare`, the **PRIMARY** provider, tried first) and the async Skyscanner fallback (`skyscanner_fare`, richest data: duration/airlines/layovers) | — |
| `RAPIDAPI_HOST` | RapidAPI host for the flights-sky API | `flights-sky.p.rapidapi.com` |
| `SKYSCANNER_CONNECT_TIMEOUT` | Connection-setup timeout (seconds) for the initial `search-roundtrip` call (the connect half of its `(connect, read)` tuple). Short so a **dead host fails fast** instead of hanging. | `10` |
| `SKYSCANNER_SEARCH_TIMEOUT` | Read / **inactivity** timeout (seconds) for the initial `search-roundtrip` call. Fires only after this many seconds of **no data arriving** — **NOT** a total wall-clock cap. A slow-but-progressing async search keeps running to completion; the app gives up only after prolonged silence. Generous default so real long-haul searches aren't killed mid-flight. | `90` |
