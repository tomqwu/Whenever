...
def _build_parser():
    ...
    p.add_argument("--from", dest="origin", required=True,
                   help="Origin city name or IATA code (e.g. Toronto or YYZ).")
    ...
    dest_group.add_argument("--city",
                            help="Single destination city name or IATA code.")
...
def _resolve_origin(raw: str) -> str:
    """Return an IATA code for the origin.  3-letter uppercase input is used as-is."""
...
def main(argv=None) -> int:
    ...
    if args.country:
        ...
        raw_city = args.city
        if len(raw_city) == 3 and raw_city.isalpha():
            iata = raw_city.upper()
            city_name = raw_city.upper()
        else:
